 ...
 */
public class FakeRuleScalaFinder {

    private FakeRuleScalaFinder() {
    }
    ...
    private static RuleFinder create(boolean findbugs, boolean fbContrib, boolean findSecBug, boolean findSecBugJsp) {
        ...
        if (findbugs) {
            RulesDefinition rulesDefinition = new org.sonar.plugins.findbugs.rules.FindbugsScalaRulesDefinition();
            rulesDefinition.define(context);
            configRuleFinderForRepo(ruleFinder, context, org.sonar.plugins.findbugs.rules.FindbugsScalaRulesDefinition.REPOSITORY_KEY);
        }

        if (fbContrib) {
            RulesDefinition rulesDefinition = new org.sonar.plugins.findbugs.rules.FbContribScalaRulesDefinition();
            rulesDefinition.define(context);
            configRuleFinderForRepo(ruleFinder, context, org.sonar.plugins.findbugs.rules.FbContribScalaRulesDefinition.REPOSITORY_KEY);
        }

        if (findSecBug) {
            RulesDefinition rulesDefinition = new org.sonar.plugins.findbugs.rules.FindSecurityBugsScalaRulesDefinition();
            rulesDefinition.define(context);
            configRuleFinderForRepo(ruleFinder, context, org.sonar.plugins.findbugs.rules.FindSecurityBugsScalaRulesDefinition.REPOSITORY_KEY);
        }
